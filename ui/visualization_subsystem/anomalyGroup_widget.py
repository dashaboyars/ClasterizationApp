...
class AnomalyGroupWidget(QWidget):
    double_clicked = pyqtSignal(object)

    def __init__(self, anomaly_group, search_query=None, parent=None):
        super().__init__(parent)
        self.parent = parent

        self.anomaly_group = anomaly_group
        ...
        self.name_label = QLabel(anomaly_group.name)
    ...
    def formatted_date(self):
        date_str = self.anomaly_group.created_at[:10]  # берем первые 10 символов
    ...
    def set_imageAmount(self):
        images_count = len(self.anomaly_group.images_list)
    ...
    def show_context_menu(self, position):
        ...
        session = ObjectsManager.find_session_by_id(self.parent, self.anomaly_group.session_id)
        MenuManager.show_anomalyGroup_contextMenu(self.parent, session, self.anomaly_group, global_pos)

    def mouseDoubleClickEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            self.double_clicked.emit(self.anomaly_group)