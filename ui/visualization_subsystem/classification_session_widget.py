...
class ClassificationSessionWidget(QWidget):
    ...
    def __init__(self, session, search_query=None, parent=None):
        ...
        # Правая часть - информация о сессии
        ...
        #Средняя уверенность
        self.medConf_label = QLabel(f"Средняя уверенность: {session.get_avg_confidence() * 100}%")
    ...
    def set_model(self):
        ...
        name = ""
        for model in self.parent.models_data[::-1]:
        ...
        self.model_label = QLabel(f"🤖 Имя модели: {name}")
        self.model_label.setStyleSheet("color: #666; font-size: 10px;")
    ...
    def show_context_menu(self, position):
        ...
        MenuManager.show_classificationSession_contextMenu(self.parent, self.session, global_pos)
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            groups = [gr for gr in self.session.images_labels.values()]
            self.double_clicked.emit(self.session, groups)